  ...
  return (
    <div className="min-h-dvh bg-transparent">
      ...
      <main className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
        <VerifiedGate>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-50 break-words">{copy.title}</h1>
          <p className="mt-2 text-xs sm:text-sm text-slate-400 break-words">
            {copy.subtitle}
          </p>
          <form onSubmit={handleSubmit} className="mt-4 sm:mt-6">
            <div className="flex flex-col gap-2 sm:flex-row">
              <input
                ...
                className="w-full rounded-xl border border-slate-800 bg-slate-900/60 px-3 py-2.5 sm:px-4 sm:py-3 text-sm text-slate-100 outline-none ring-red-500/0 transition focus:border-red-500/50 focus:ring-2 focus:ring-red-500/20 h-11 sm:h-auto break-words"
              />
              <button
                type="submit"
                disabled={pending}
                className="rounded-xl bg-red-600 px-4 py-2.5 sm:py-3 text-sm font-semibold text-white hover:bg-red-500 disabled:opacity-60 sm:min-w-28 h-11 sm:h-auto flex-shrink-0"
              >
